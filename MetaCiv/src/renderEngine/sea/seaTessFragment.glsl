...
void main(void) {
	...
	distortion2 += (texture(dudvMap, vec2(fs_in.tc.x*50.0 - moveFactor* 2.0,fs_in.tc.y*50.0 - moveFactor* 2.0)).rg * 2.0 -1.0) * waveStrenght;
	vec3 normal3 =  normalize((texture(normalMap, distortion2).rbg* fs_in.height*2 ) - vec3(1.0));
