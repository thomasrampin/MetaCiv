...
public class Objet implements Serializable {
	...
	public Objet(String name,int qualite,ArrayList<Effect> effets)
	{
		nom = name;
		icone = Configuration.getIcon(defaultIconName);
		nomIcone = defaultIconName;
	...
	public Objet(String name,int qualite, Effect effet)
	{
		this.effets = new ArrayList<Effect>();
		this.effets.add(effet);
		this.recette = new ArrayList<Objet>();
	...
	public void enregistrer(File cible) {
		PrintWriter out;
	//	System.out.println("Enregistrement de l'objet "+getNom());
		try {
			...
			out.print("Effects : ");
	//		System.out.println("Nombre d'effets enregistres "+this.effets.size());
			if(this.effets.size() > 0)
			{
				for(int i = 0; i < this.effets.size();++i)
				{
	//				System.out.println("     Il a l'effet "+this.getEffets().get(i).getName());
