...
public class PanelInspecteur extends JPanel{
	...
	public PanelInspecteur(Human h) {
		this();
		this.h = h;
		fixedHuman = true;
		actualiser();
	...
	{
		if ((select(agentID) != null && select(agentID).getClass() == Human.class))
		{
			((Human) select(agentID)).setIsSelected(false);
		}
		
		agentID++;
		int iteration = 0;
		while ((select(agentID) == null || select(agentID).getClass() != Human.class) && iteration <= 50000)
		{
			agentID++;
			iteration++;
			if (agentID >= 50000)
	...
	{
		if (!fixedHuman) h = (Human) select(agentID);
		if (h != null) {
...
//			donnees[0][1] = h.hashCode();
	...
	private Turtle select(int id) {
		if (World.getInstance().getTick() > 2 && World.getInstance().getTurtlesWithRoles(DefineConstants.Role_Human).size() > id) return (Human) World.getInstance().getTurtlesWithRoles(DefineConstants.Role_Human).get(id);
		return null;
	}
