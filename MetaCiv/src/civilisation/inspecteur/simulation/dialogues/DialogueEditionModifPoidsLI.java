...
public class DialogueEditionModifPoidsLI extends JDialog implements ActionListener, PropertyChangeListener{
	...
	public DialogueEditionModifPoidsLI(Frame f , boolean modal, GCogniton gCogniton){
		super(f,modal);
			...
			if (optionPane.getValue().equals("OK")){
				...
				for (int i = 0; i < plansLies.size();i++){
					if (!plansLies.get(i).getSelectedItem().equals("--AUCUN--")){
						System.out.println(plansLies.get(i).getSelectedIndex()-1 +" : "+ Configuration.getSchemaCognitifEnCourEdition().getPlans().size());
						LienPlan lien = new LienPlan(  Configuration.getSchemaCognitifEnCourEdition().getPlans().get(plansLies.get(i).getSelectedIndex()-1),(Integer)poids.get(i).getValue());
	...
	public void ajouterBox(NPlan plan, int startValue){
		...
		box.addItem("--AUCUN--");
		for (int j = 0; j < Configuration.getSchemaCognitifEnCourEdition().getPlans().size(); j++){
			box.addItem(Configuration.getSchemaCognitifEnCourEdition().getPlans().get(j));
			if (Configuration.getSchemaCognitifEnCourEdition().getPlans().get(j).equals(plan)){
