	...
	public DialogueEditerLiensConditionnels(Frame f , boolean modal, GCogniton gCogniton){
		...
		for (int i = 0; i < gCogniton.getCogniton().getPlansAutorises().size(); i++){
			...
			for (int j = 0; j < Configuration.getSchemaCognitifEnCourEdition().getPlans().size(); j++){
				...
				if (Configuration.getSchemaCognitifEnCourEdition().getPlans().get(j).equals(gCogniton.getCogniton().getPlansAutorises().get(i))){
		...
		array.add(new JLabel(I18nList.CheckLang("Conditionned plan :")));
			...
			if (optionPane.getValue().equals("OK")){
				...
				for (int i = 0; i < plansLies.size();i++){
					if (!plansLies.get(i).getSelectedItem().equals("--AUCUN--")){
						...
						nouveauxLiens.add(Configuration.getSchemaCognitifEnCourEdition().getPlans().get(plansLies.get(i).getSelectedIndex()-1));
					}
				}
