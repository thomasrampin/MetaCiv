...
public class GCarte extends JJComponent{
	...
	public void pinceauPatch(MouseEvent e) {
		int taille = panelEnvironnement.getTaillePseudoPatch();
		Terrain terrain = (Terrain) panelEnvironnement.getPanelPrincipal().getPanelTerrains().getListeTerrains().getSelectedValue();
		if (terrain != null){ 
			int x = (int)(e.getX()/(double)taille);
			...
			for(int i = -1; i < 2;++i)
			{
				for(int j = -1; j < 2;++j)
				{
					if(panelEnvironnement.getCarte().size() > y+j && y+j >= 0 && panelEnvironnement.getCarte().get(y+j).size() > x+i && x+i >= 0)
					panelEnvironnement.getCarte().get(y+j).get(x+i).setTerrain(terrain);
				}
			}
