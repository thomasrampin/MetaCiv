...
public class A_Mendicity extends Action{
	...
	public Action effectuer(Human h) {
		...
		List<Human> humains = h.humansHereWithCogniton(targetCogniton);
		
		//Do the trade
		if (!humains.isEmpty()) {
						
			Human target = humains.get((int) Math.floor(humains.size() * Math.random()));
			...
			Human cible = h.getNearestTurtleOf(World.getInstance().getHumansWithCogniton(targetCogniton));
